...
def plot_diagnostics(datapath, tag="global"):
    ...
    d = np.load(datapath)
    # Mask to keep only actively convecting models
    mask_conv = d['Nu'] > 1.0
    
    H     = d['H_km'][mask_conv]
    q_bas = d['q_basal'][mask_conv] * 1e3              # → mW/m²
    q_int = d['q_tidal_integrated'][mask_conv] * 1e3   # → mW/m²
    T_pr  = d['T_profiles'][mask_conv].copy()          # (n, nx)
    z_pr  = d['z_profiles'][mask_conv]                 # (n, nx) km
    q_pr  = d['q_profiles'][mask_conv] * 1e6           # → μW/m³
    k_pr  = d['k_profiles'][mask_conv]                 # W/(m·K)
    D_cond = d['D_cond_km'][mask_conv]
    D_conv = d['D_conv_km'][mask_conv]
    Nu     = d['Nu'][mask_conv]
    n = len(H)
    if n == 0:
        print(f"No convecting models found in {datapath}!")
        return
    nx = T_pr.shape[1]
    ...
    fig.suptitle(f"Diagnostic Heating Profiles — Convecting {tag.title()} (N={n:,})",
                 fontsize=15, y=1.01)
    fig.tight_layout()
    outname = f"diagnostic_profiles_{tag}_conv.png"
...
def plot_viscosity(datapath, tag="global"):
    ...
    d = np.load(datapath)
    # Mask for convecting only
    mask_conv = d['Nu'] > 1.0
    
    H      = d['H_km'][mask_conv]
    T_pr   = d['T_profiles'][mask_conv]         # (n, nx)  K
    eta_pr = d['eta_profiles'][mask_conv]       # (n, nx)  Pa·s
    D_cond = d['D_cond_km'][mask_conv]
    D_conv = d['D_conv_km'][mask_conv]
    d_grain = d['d_grain'][mask_conv]           # (n,)  m
    Nu      = d['Nu'][mask_conv]
    n  = len(H)
    if n == 0:
        return
    ...
    fig.suptitle(f"Viscosity Diagnostic — Convecting {tag.title()} (N={n:,})",
                 fontsize=15, y=1.01)
    fig.tight_layout()
    outname = f"viscosity_profiles_{tag}_conv.png"
    fig.savefig(os.path.join(FIGURES_DIR, outname), dpi=200, bbox_inches="tight")
    print(f"  Saved to figures/{outname}")
...
def plot_diagnostics(datapath, tag="global"):
    ...
    H = d["H_km"][mask_conv]
    T_pr = d["T_profiles"][mask_conv].copy()
    z_pr = d["z_profiles"][mask_conv]
    q_pr = d["q_profiles"][mask_conv] * 1e6
    D_cond = d["D_cond_km"][mask_conv]
    Nu = d["Nu"][mask_conv]
    n = len(H)
    if n == 0:
    ...
    save_fig(fig, f"diagnostic_profiles_{tag}_conv", FIGURES_DIR)
...
def plot_viscosity(datapath, tag="global"):
    ...
    H = d["H_km"][mask_conv]
    T_pr = d["T_profiles"][mask_conv]
    eta_pr = d["eta_profiles"][mask_conv]
    n = len(H)
    if n == 0:
        print(f"No convecting models found in {datapath}!")
        return
    ...
    fig.suptitle(f"Viscosity diagnostics - Convecting {tag.title()} (N = {n:,})",
                 fontsize=9, y=1.03)
    fig.tight_layout(w_pad=1.6)
    save_fig(fig, f"viscosity_profiles_{tag}_conv", FIGURES_DIR)

...
_SAMPLER_MAP = {
    ...
    "AuditedShellSampler":      AuditedShellSampler,
}
...
def _andrade_reference_viscosity(mu_ref, omega):
    ...
    x = np.logspace(-6, 6, 4000)  # x = omega * tau = omega * eta / mu
...
def plot_diagnostics(datapath, tag="global"):
    ...
    d = np.load(datapath)
    H     = d['H_km']
    q_bas = d['q_basal'] * 1e3              # → mW/m²
    q_int = d['q_tidal_integrated'] * 1e3   # → mW/m²
    T_pr  = d['T_profiles'].copy()          # (n, nx)
    z_pr  = d['z_profiles']                 # (n, nx) km
    q_pr  = d['q_profiles'] * 1e6           # → μW/m³
    k_pr  = d['k_profiles']                 # W/(m·K)
    D_cond = d['D_cond_km']
    D_conv = d['D_conv_km']
    Nu     = d['Nu']
    n = len(H)
    ...
    fig.suptitle(f"Diagnostic Heating Profiles — {tag.title()} (N={n:,})",
                 fontsize=15, y=1.01)
    fig.tight_layout()
    outname = f"diagnostic_profiles_{tag}.png"
...
def plot_viscosity(datapath, tag="global"):
    ...
    d = np.load(datapath)
    H      = d['H_km']
    T_pr   = d['T_profiles']         # (n, nx)  K
    eta_pr = d['eta_profiles']       # (n, nx)  Pa·s
    D_cond = d['D_cond_km']
    D_conv = d['D_conv_km']
    d_grain = d['d_grain']           # (n,)  m
    Nu      = d['Nu']
    n  = len(H)
    nx = T_pr.shape[1]
    ...
    fig.suptitle(f"Viscosity Diagnostic — {tag.title()} (N={n:,})",
                 fontsize=15, y=1.01)
    fig.tight_layout()
    outname = f"viscosity_profiles_{tag}.png"
    fig.savefig(os.path.join(FIGURES_DIR, outname), dpi=200, bbox_inches="tight")
    print(f"  Saved to figures/{outname}")
...
def plot_diagnostics(datapath, tag="global"):
    ...
    save_fig(fig, f"diagnostic_profiles_{tag}", FIGURES_DIR)
...
def plot_viscosity(datapath, tag="global"):
    ...
    fig.suptitle(f"Viscosity diagnostics - {tag.title()} (N = {n:,})",
                 fontsize=9, y=1.03)
    fig.tight_layout(w_pad=1.6)
    save_fig(fig, f"viscosity_profiles_{tag}", FIGURES_DIR)


if __name__ == "__main__":
    mp.freeze_support()

    # Default: audited 2026 global sampler
    sampler_cls = AuditedShellSampler
    npz_name = "diagnostic_profiles.npz"
    tag = "global"
    default_n = 10000
    ...
    if os.path.exists(outpath) and "--run" not in sys.argv:
        print(f"Using existing results (pass --run to re-compute)")
    else:
